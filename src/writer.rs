...
pub fn write_metadata(global: &GlobalMeta, tracks: &[TrackMeta]) -> anyhow::Result<()> {
    for track in tracks {
        ...
        let album = global.album.as_deref().unwrap_or("Unknown Album");
        let album_artist_str = global.album_artist.join("; ");
        ...
        let publisher = global.publisher.as_deref().unwrap_or("");
        let copyright = global.copyright.as_deref().unwrap_or("");
        ...
        // Using kid3-cli - simple set commands (working approach)
